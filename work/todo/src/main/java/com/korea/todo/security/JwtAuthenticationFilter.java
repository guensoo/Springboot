...
public class JwtAuthenticationFilter extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			// parseBearerToken메서드
			// HTTP 요청 헤더에서 Authorization값을 가져와 Bearer 토큰 형식인지
			// 확인한 후, 토큰을 가져온다.
			// 토큰이 없거나, 유요하지 않으면 null을 반환
			...
			if(token != null && !token.equalsIgnoreCase("null")) {
				...
				// 생성자의 첫번쨰 인자 : 인증 주체를 나타내는 객체(유저id, username)
				...
				// buildDetails(request) : HttpServletRequest객체에서 인증과 관련된 추가적인 정보를
				// 추출해 WebAuthenticationDetails 객체를 반환
				...
				// 다른 api에서 SecurityContextHolder.getContext()를 통해
				// 방금 등록한 정보를 볼 수 있다.
