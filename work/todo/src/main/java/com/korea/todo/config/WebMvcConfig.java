...
// WebMvcConfigurer 스프링 MVC의 기본 설정을 내가 조절하고 싶을 때 사용한다.
// 스프링 MVC는 여러가지 기본설정(뷰 리졸버, 메시지 컨버터, CORS등)을 가지고 있다.
public class WebMvcConfig implements WebMvcConfigurer{
	private final long MAX_AGE_SECS = 3600; // 브라우저가 CORS 요청 결과를 캐싱하는 최대시간 설정
	...
	public void addCorsMappings(CorsRegistry registry) {
		...
		// .allowedMethods("GET", "PUT", "POST", "DELETE") : HTTP메서드를 허용하겠다.
		.allowedMethods("GET", "PUT", "POST", "DELETE")
		// .allowedHeaders("*") : 모든 헤더를 허용하겠다.
		.allowedHeaders("*")
		// .allowCredentials(true) : 쿠키나 인증 정보를 포함한 요청을 허용
		.allowCredentials(true)
		// .maxAge(MAX_AGE_SECS) : 브라우저가 서버로부터 받은 응답을 일정 시간 동안 저장해두고,
		// 그 시간 내에 동인한 요청이 있을 경우 서버에 다시 요청하지 않고,
		// 저장된 응답을 재사용한다. 
		.maxAge(MAX_AGE_SECS);
	}
